...
import G6, { Graph, INode, LayoutConfig, NodeConfig } from '@antv/g6';
import { GForceLayout } from '@antv/layout';
import insertCss from 'insert-css';
import louvain from '@antv/algorithm/lib/louvain';
import { IEdge, Item, ModelConfig } from '@antv/g6-core/lib/types';
import { ClusterData } from '@antv/algorithm/lib/types';
import { LargeGraphData, LargeGraphEdge, LargeGraphNode, NodeClickPayload, EdgeClickPayload } from '../interfaces';
import {
  colorSets,
...
import './custom-line';
import './custom-quadratic';
import './real-node';
import { Log } from '@idg/idg';

const TAG = 'largegraph/LargeGraph';
...
export default class LargeGraph extends Vue {
   ...
   *
   * @param {INode} item
   * @returns {NodeClickPayload}
   * @memberof LargeGraph
   */
  @Emit('nodeClick')
  handleNodeClick(item: INode): NodeClickPayload {
    const model = item.getModel() as NodeConfig;

    const res: NodeClickPayload = {
      id: model.id,
    };

    Log.debug(TAG, 'handleNodeClick', res, item, this.nodeMap, this.data);
    return res;
  }
   ...
   *
   * @param {IEdge} item
   * @returns {EdgeClickPayload}
   * @memberof LargeGraph
   */
  @Emit('edgeClick')
  handleEdgeClick(item: IEdge): EdgeClickPayload {
    console.log(item.getModel());
    const res: EdgeClickPayload = {
      id: item.getModel().id,
      source: item.getSource().getModel() as NodeConfig,
      target: item.getTarget().getModel() as NodeConfig,
      label: item.getModel().label.toString(),
    };
    Log.debug(TAG, 'handleEdgeClick', res, item, this.nodeMap, this.data);
    return res;
  }
  ...
  @Watch('expandLevel')
  watchExpandLevel() {
    const level = Number(this.expandLevel);
    if (level) {
      this.handleExpandLevel(level);
    }
  }

  private handleExpandLevel(level: number) {
    if (level === 1) {
      this.nodeAllColapse();
    } else if (level === 2) {
      this.nodeExpand(this.clusteredData);
    } else {
      this.nodeExpand(this.clusteredData);
      this.expandNeighbor(this.clusteredData);
    }
  }

  private nodeExpand(clusteredData: ClusterData) {
    const nodes: INode[] = this.graph.getNodes();
    const graph_length: number = this.graph.getNodes().length;
    let mixedGraphData: { nodes: any; edges: any };

    nodes.forEach((item) => {
      const model = item.getModel();
      const newArray = this.manageExpandCollapseArray(graph_length, model, this.collapseArray, this.expandArray);
  ...
  private expandNeighbor(clusteredData: ClusterData) {
    const nodes: INode[] = this.graph.getNodes();
    const expandNeighborSteps = this.expandLevel - 2;
    let mixedGraphData: { nodes: any; edges: any };
  ...
  private manageExpandCollapseArray(nodeNumber: number, model: any, collapseArray: any[], expandArray: any[]) {
  ...
  private examAncestors = (model: { parentId: any }, expandedArray: any[], length: number, keepTags: boolean[]) => {
  ...
  private getNeighborMixedGraph(
    ...
    // update the manipulate position for center gravity of the new nodess
  ...
  private handleRefreshGraph(
    ...
    options?: { edgeLabelVisible?: boolean; isNewGraph?: boolean; changeSize?: boolean },
  ) {
